...
hi_s32 sample_parse_cmd(hi_void *wdata, hi_char *cmd, ssize_t len, hi_void *msg)
{
    hi_u8                  cmd_id;
    hi_u32                 off_set = 0;
    hi_char                wlan_name[LUX_CMD_MAX_LEN] = {0};
    if (cmd == HI_NULL) {
        return HI_FAILURE;
    }

    if (sample_get_cmd_one_arg(cmd, wlan_name, LUX_CMD_MAX_LEN, &off_set) != HI_SUCCESS) {
        return HI_FAILURE;
    }
    cmd += (off_set + 1);

    for (cmd_id = 0; cmd_id < g_cmd_com.count; cmd_id++) {
        if (strncmp(g_cmd_com.cmd_tbl[cmd_id].cmd_name, wlan_name, strlen(g_cmd_com.cmd_tbl[cmd_id].cmd_name)) == 0) {
            sample_message_s *pmsg = (sample_message_s *)msg;
            pmsg->what = g_cmd_com.cmd_tbl[cmd_id].cmd_id;
            pmsg->len = len - off_set - 1;
            (hi_void)memcpy(pmsg->obj, cmd, pmsg->len);
            //pmsg->obj[LUX_CMD_MAX_LEN - 1] = '\0';
...
hi_s32 sample_sock_cmd_entry(hi_void *wdata, const char *cmd, ssize_t len, hi_void *msg)
{
    ...
    if (len > LUX_CMD_MAX_LEN) {
        sample_log_print("command len > %d!\n", LUX_CMD_MAX_LEN);
        return HI_FAILURE;
    }
    pcmd = malloc(LUX_CMD_MAX_LEN);
    if (pcmd == HI_NULL) {
        return HI_FAILURE;
    }
    if (cmd != HI_NULL) {
