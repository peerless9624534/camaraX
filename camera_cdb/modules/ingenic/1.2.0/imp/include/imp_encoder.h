...
typedef enum {
	...
	IMP_ENC_OPT_L2_CACHE          = 0x00000800,
...
typedef enum {
	IMP_ROI_QUALITY_HIGH = -10,
	IMP_ROI_QUALITY_MEDIUM = -5,
	IMP_ROI_QUALITY_LOW = +10,
	IMP_ROI_QUALITY_DONT_CARE = +25,
	IMP_ROI_QUALITY_STATIC = 0x80,
	IMP_ROI_QUALITY_INTRA = 0x40,
	IMP_ROI_QUALITY_MAX_ENUM,
} IMPEncoderRoiQuality;

typedef struct {
	uint32_t    iPosX;        /* 左上角横坐标, HEVC:32对齐 AVC:16对齐 */
	uint32_t    iPosY;        /* 左上角纵坐标, HEVC:32对齐 AVC:16对齐 */
	uint32_t    iWidth;       /* ROI区域宽度, HEVC:32对齐 AVC:16对齐 */
	uint32_t    iHeight;      /* ROI区域高度, HEVC:32对齐 AVC:16对齐 */
	IMPEncoderRoiQuality iQuality;    /* ROI区域质量 */
} IMPEncoderRoiParam;

typedef struct {
	int                   index;      /* 索引号, 可取[0, 7], 每个ROI对应一个index */
	bool                  enable;     /* ROI区域开关 */
	IMPEncoderRoiParam    RoiParam;   /* ROI区域属性 */
} IMPEncoderRoiAttr;

typedef struct {
	bool					enRoi;        /* 是否使能编码roi */
	IMPEncoderRoiQuality	backQuality;  /* 背景质量(如果不设置会使用默认质量) */
} IMPEncoderRoiCfg;
...
typedef struct {
	...
	IMPEncoderRoiCfg 		roi;           /**< 编码器区域质量属性 */
} IMPEncoderEncAttr;
 ...
 */
typedef enum {
	IMP_ENC_SEI_MODE_NONE       = 0x00,  /**< 关闭SEI */
	IMP_ENC_SEI_MODE_INTRA      = 0x01,  /**< 应用SEI到I帧 */
	IMP_ENC_SEI_MODE_ALL        = 0x02,  /**< 应用SEI到所有帧 */
} IMPEncoderSEIMode;
 ...
 */
typedef struct {
	IMPEncoderSEIMode SeiMode;	     /**< SEI模式 */
	uint8_t* uuid;			     /**< uuid,协议规定长度固定为16Byte,常用于数据校验 */
	uint8_t* userData;		     /**< 用户自定义信息 */
	uint8_t  userDataSize;		     /**< 用户自定义信息长度,最大支持128byte */
} IMPEncoderSeiAttr;
 ...
 *
 * @param[in] encChn 编码Channel号,取值范围: [0, @ref NR_MAX_ENC_CHN - 1]
 * @param[in] iMinQP iMinQP值,取值范围: [0, 51]
 * @param[in] iMaxQP iMaxQP值,取值范围: [0, 51]
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
...
/**
 * @brief IMP_Encoder_SetChnSeiAttr(int encChn, IMPEncoderSeiAttr *SeiAttr);
 *
 * 配置SEI信息属性.
 *
 * @param[in] encChn       通道编号.
 * @param[out] SeiAttr     SEI属性.
 *
 * @retval  =0	 		   成功.
 * @retval  <0                     失败.
 *
 ...
 * @attention 此API只适用于H264和h265编码channel.
 */
int IMP_Encoder_SetChnSeiAttr(int encChn, IMPEncoderSeiAttr *SeiAttr);

/**
 * @brief IMP_Encoder_GetChnSeiAttr(int encChn, IMPEncoderSeiAttr *SeiAttr);
 *
 * 获取SEI信息属性.
 *
 * @param[in] encChn       通道编号.
 ...
 * @attention 此API只适用于H264和h265编码channel.
 */
int IMP_Encoder_GetChnSeiAttr(int encChn, IMPEncoderSeiAttr *SeiAttr);
 ...
 * @remarks 此API必须在通道创建之后调用.
 *
 * @attention 仅仅在单帧调度下生效.
 */
int IMP_Encoder_SetFrameRelease(int encChn, int num, int den);

/**
 * @brief IMP_Encoder_SetChnRoiAttr(int encChn, IMPEncoderRoiAttr *RoiAttr);
 *
 * 动态设置ROI属性.
 *
 * @param[in] encChn        通道编号.
 * @param[in] RoiAttr       ROI属性.
 *
 * @retval  >=0 && < 32     成功.
 * @retval  <0              失败.
 *
 * @remarks   此API必须在通道创建之后调用.
 *
 * @attention 确保通道属性enRoi使能后才会有效果.
 */
int IMP_Encoder_SetChnRoiAttr(int encChn, IMPEncoderRoiAttr *RoiAttr);

/**
 * @brief IMP_Encoder_GetChnRoiAttr(int encChn, IMPEncoderRoiAttr *RoiAttr);
 *
 * 获取上一次设置成功的ROI属性.
 *
 * @param[in] encChn        通道编号.
 * @param[out] RoiAttr      ROI属性.
 *
 * @retval  >=0 && < 32     成功.
 * @retval  <0              失败.
 *
