...
/* Warning, this struct must be same as IMPNCUFrameInfo in file src/imp/include/isp/ncu_common.h */
typedef struct c_ncu_frame_info {
	uint32_t width;
	uint32_t height;
	uint32_t frm_num;
...
typedef struct c_img {
	...
	/* usage from t30, why pointer, becase thi ncu_frame_info's life be same as isp frame */
