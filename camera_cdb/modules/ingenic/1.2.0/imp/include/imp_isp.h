...
#define GPIO_PD(n) (3 * 32 + (n))
	...
	*/
	 ...
	 */
	typedef struct {
		uint8_t enable;			/**< bypass使能 */
		uint8_t vc_index;		/**< mipi虚拟通道选择(0/1/2/3) */
		uint8_t dma_index;		/**< dma通道选择(0/1) */
	} IMPISPBypass;
	 ...
	 */
	typedef struct {
		IMPISPBypass bypass[3];	/**< 各通道bypass属性 */
	} IMPISPBypassAttr;
	 ...
	 *
	 * @param[in] num       对应sensor的标号
	 * @param[in] attr		bypass属性
	 *
	 * @retval 0 成功
	 * @retval 非0 失败，返回错误码
	 *
	 * @remark dma应优先使用通道0。
	 * @remark 使用第三路摄像头时，只能使用通道0。
	 *
	 * @attention 在使用这个函数之前，IMP_ISP_Open已被调用。
	 * @attention 该函数与IMP_ISP_Tuning_SetISPBypass不能同时使用.
	 */
