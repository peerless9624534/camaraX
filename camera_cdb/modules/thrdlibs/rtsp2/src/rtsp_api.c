...
static int rtcp_try_tx_sr (struct rtp_connection *c, uint64_t ntptime_of_zero_ts, uint64_t ts, uint32_t sample_rate)
{
	...
	if (c->rtcp_last_ts && ts < c->rtcp_last_ts + 5000ULL) {
		return 0;
	}

	ntpts_msw = (uint32_t)((ntptime_of_zero_ts + ts) / 1000ULL);
	ntpts_lsw = (uint32_t)(((ntptime_of_zero_ts + ts) % 1000ULL) * (1ULL << 32) / 1000ULL);
	rtp_ts = (uint32_t)(ts * sample_rate / 1000ULL);
