...
soap_match_namespace(struct soap *soap, const char *id1, const char *id2, size_t n1, size_t n2)
{
  ...
  while (np && (strncmp(np->id, id1, n1) || np->id[n1]))
    np = np->next;
  //printf("soap_match_namespace_1_ns:%s\n", np->ns);
  //printf("soap_match_namespace_2_id:%d\n", np->id[0]);
  //printf("soap_match_namespace_3_index:%d\n", np->index);
  if (np)
  {
    ...
    if (s)
    {
      ...
      if (t != tag2 && !(soap->mode & SOAP_XML_IGNORENS))
      {
        ...
        if (err)
        {
          ...
          {
              return SOAP_OK;
             /* return SOAP_TAG_MISMATCH;*//*严重问题临时注释，后期必须解决！！！！！！！！！！！（author：hkj）*/
          }
          return err;
        }
      }
...
http_response(struct soap *soap, int status, ULONG64 count)
{
  ...
  if (status == 401)
  {
    ...
    soap_wsse_rand_nonce(nonce, 20);
    nonceBase64 = soap_s2base64(soap, (unsigned char*)nonce, NULL, 20);
    (SOAP_SNPRINTF_SAFE(soap->tmpbuf, sizeof(soap->tmpbuf)), "Digest qop=\"auth\" realm=\"%s\" nonce=\"%s\"", (soap->authrealm && strlen(soap->authrealm) + 14 < sizeof(soap->tmpbuf)) ? soap->authrealm : "gSOAP Web Service", nonceBase64);
...
soap_peek_element(struct soap *soap)
{
  ...
  *s = '\0';
  ...
  //printf("*********soap_peek_element**********_index:%d\n", soap->nlist->index);
  //printf("*********soap_peek_element**********_ns:%s\n", soap->nlist->ns);
  //printf("*********soap_peek_element**********_id 0:%d\n", soap->nlist->id[0]);
  //printf("*********soap_peek_element**********_id_1:%d\n", soap->nlist->id[1]);
#ifdef WITH_DOM
  if (att)
  {
...
/* On some systems strtof requires -std=c99 or does not even link: so we try strtod first */
        char* r;
        *p = (float)atof(s);
        DBGLOG(TEST, SOAP_MESSAGE(fdebug, "%s,%d:*p==%fENTER-----------------3\n", __FILE__, __LINE__, *p));


  /*转化函数有问题将下面代码屏蔽，使用上面自己所写的函数进行数据转化；author：hkj*/   
#if 0
#if defined(WITH_C_LOCALE)
